...
void basketball1::moveTo(int x, int y, int color)
{
	HANDLE hout;
	COORD pos;
	pos.X = x;
...
//播放音乐
void basketball1::music(const char file[])
{
	PlaySoundA(file, NULL, SND_FILENAME | SND_ASYNC);
}

//初始化球
void basketball1::initBall(int color)
{
...
//显示球
void basketball1::showBall(int color)
{
...
//与障碍物的碰撞判断
void basketball1::judgeBoard(pf clearBoard, vector<vector<int>> &obstacle)
{
	vector<int> temp;
...
//与撞球的人的碰撞判断
void basketball1::judgeCXK(const int &CXKx, const int &CXKy, int &state)
{
	if (ballY == 25)
	{
...
//纵坐标上的边界判断
void basketball1::judgeY(int &state)
{
	if (ballY <= 0)
	{
		ballY = 1;
...
//判断球与球之间的碰撞
void basketball1::judgeBall(const int &x, const int &y, const int &offx, const int &offy)
{
...
void basketball1::moveBall(const int &CXKx, const int &CXKy, int &state, pf clearBoard, vector<vector<int>> &obstacle)
{
	ballX += xOffset;
	judgeX();
	ballY += yOffset;
...
int basketball1::xOffset = 1;
int basketball1::yOffset = 1;
int basketball1::ballX;
int basketball1::ballY;
